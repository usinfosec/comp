...
export class PrismaExtension implements BuildExtension {
	...
	async onBuildComplete(context: BuildContext, manifest: BuildManifest) {
		...
		if (this.options.typedSql) {
			...
			const sqlFiles = await readdir(join(prismaDir, "sql")).then(
				(files) => files.filter((file) => file.endsWith(".sql")),
			);
			context.logger.debug("Found sql files", {
				sqlFiles,
			});
			const sqlDestinationPath = join(
				manifest.outputPath,
				"prisma",
				"sql",
			);
		...
		if (usingSchemaFolder) {
			...
			try {
				...
				context.logger.debug(
					"Found prisma files in the schema subfolder",
					{
						subDirPrismaFiles,
					},
				);
				...
				context.logger.debug(
					`No 'schema' subfolder found in ${schemaDir}`,
				);
			}
