...
export const _upsertOrgFrameworkStructureCore = async ({
    ...
    |--------------------------------------------------
    */
    const requirementIds = frameworkEditorFrameworks.flatMap(
    ...
    */
    ...
    const existingFrameworkInstanceFrameworkIds = new Set(existingFrameworkInstances.map(fi => fi.frameworkId));

    const frameworkInstancesToCreateData = frameworkEditorFrameworks
        .filter(f => targetFrameworkEditorIds.includes(f.id) && !existingFrameworkInstanceFrameworkIds.has(f.id))
        .map(framework => ({
            organizationId: organizationId,
    ...
    const allOrgFrameworkInstances = await tx.frameworkInstance.findMany({
        where: { organizationId: organizationId, frameworkId: { in: targetFrameworkEditorIds } },
    ...
    */
    ...
    for (const controlTemplateRelation of groupedControlTemplateRelations) { 
        const newControlId = controlTemplateIdToInstanceIdMap.get(controlTemplateRelation.controlTemplateId);

        if (!newControlId) {
            console.warn(`UpsertOrgFrameworkStructureCore: Control instance not found for template ID ${controlTemplateRelation.controlTemplateId}. Skipping relation processing.`);
            continue;
        }
        ...
        if (controlTemplateRelation.requirementTemplateIds.length > 0) {
            for (const reqTemplateId of controlTemplateRelation.requirementTemplateIds) {
                ...
                if (frameworkInstanceId) {
                    ...
                    console.warn(`UpsertOrgFrameworkStructureCore: Could not find FrameworkInstanceId for editor requirement ID ${reqTemplateId}. Cannot create RequirementMap for Control ${newControlId}.`);
                }
            }
        }
        ...
        if (controlTemplateRelation.policyTemplateIds.length > 0) {
            ...
            for (const policyTemplateId of controlTemplateRelation.policyTemplateIds) {
                ...
                if (newPolicyId) {
                    ...
                    console.warn(`UpsertOrgFrameworkStructureCore: Policy instance not found for template ID ${policyTemplateId}. Cannot connect to Control ${newControlId}.`);
                }
            }
        ...
        if (controlTemplateRelation.taskTemplateIds.length > 0) {
            ...
            for (const taskTemplateId of controlTemplateRelation.taskTemplateIds) {
                ...
                if (newTaskId) {
                    ...
                    console.warn(`UpsertOrgFrameworkStructureCore: Task instance not found for template ID ${taskTemplateId}. Cannot connect to Control ${newControlId}.`);
                }
            }
    ...
    console.log("UpsertOrgFrameworkStructureCore: Processed data based on source framework editor IDs:", frameworkEditorFrameworks.map(f => f.id));
    // console.log(JSON.stringify({ groupedControlTemplateRelations }, null, 2)); // Optional: for deeper debugging
...
export const initializeOrganization = async ({frameworkIds, organizationId}: InitializeOrganizationInput) => {
    ...
    if (frameworksAndReqsToProcess.length === 0 && frameworkIds.length > 0) {
        console.warn(`InitializeOrganization: No FrameworkEditorFrameworks found for IDs: ${frameworkIds.join(', ')}`);
    }
    
    const result = await db.$transaction(
        async (tx) => {
            // Use the renamed core function
