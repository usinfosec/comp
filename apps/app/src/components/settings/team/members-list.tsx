...
export function MembersList({ members, currentUserRole, hasOrganization }: MembersListProps) {
  const t = useI18n();
  ...
  if (!hasOrganization) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t("settings.team.members.empty.no_organization.title")}</CardTitle>
          <CardDescription>{t("settings.team.members.empty.no_organization.description")}</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  if (members.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t("settings.team.members.empty.no_members.title")}</CardTitle>
          <CardDescription>{t("settings.team.members.empty.no_members.description")}</CardDescription>
        </CardHeader>
      </Card>
    );
  }
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-3">
          {members.map((member) => {
            const isCurrentUser = member.userId === members.find(m => m.role === currentUserRole)?.userId;
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-medium flex flex-col space-y-1">
                      <span className="text-sm">{member.user.name || member.user.email} <span className="text-xs text-muted-foreground">({member.user.email})</span></span>
