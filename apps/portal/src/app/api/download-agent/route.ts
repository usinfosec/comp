...
exit 0`;
  ...
  const pkgKey = `${orgId}/macos/fleet-osquery.pkg`;
  const s3Object = await s3Client.send(
    new GetObjectCommand({ Bucket: "compai-fleet-packages", Key: pkgKey })
  );

  if (!s3Object.Body) {
    logger("S3 object body is empty for key", { pkgKey });
    return new NextResponse(`Could not retrieve package from S3: ${pkgKey}`, {
      status: 500,
    });
  }
