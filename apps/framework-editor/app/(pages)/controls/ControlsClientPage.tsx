...
const controlsSearchableKeys: ControlsPageSortableColumnKey[] = [
	"name",
	"description",
];
...
export function ControlsClientPage({
	...
	const initialGridData: ControlsPageGridData[] = useMemo(() => {
		return initialControls.map((control) => {
			...
			return {
				...
				taskTemplates:
					control.taskTemplates?.map((tt) => ({ id: tt.id, name: tt.name })) ??
					[],
	...
	const filteredDataByFramework = useMemo(() => {
		if (
			selectedFramework === "" ||
			selectedFramework === ALL_FRAMEWORKS_OPTION_VALUE
		) {
			return dataForGrid;
		}
		return dataForGrid.filter((control) =>
			control.requirements?.some(
				(req) => req.frameworkName === selectedFramework,
			),
		);
				...
				>
					<>
						...
						>
							...
							<SelectContent>
								<SelectItem value={ALL_FRAMEWORKS_OPTION_VALUE}>
									All Frameworks
								</SelectItem>
