...
import { z } from 'zod';
import { FrameworkEditorPolicyTemplate, Frequency, Departments } from '@prisma/client';
...
export function EditPolicyDialog({ policy, isOpen, onClose, onPolicyUpdated }: EditPolicyDialogProps) {
                                        ...
                                        <SelectContent>
                                            {Object.values(Frequency).map((freq) => (
                                                <SelectItem key={freq} value={freq} className="rounded-sm">
                                                    {freq.charAt(0).toUpperCase() + freq.slice(1).toLowerCase().replace('_',' ')}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                        ...
                                        <SelectContent>
                                            {Object.values(Departments).map((dept) => (
                                                <SelectItem key={dept} value={dept} className="rounded-sm">
                                                    {dept.charAt(0).toUpperCase() + dept.slice(1).toLowerCase().replace('_',' ')}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
