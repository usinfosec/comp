...
export async function updatePolicyTemplateDetails(
...
): Promise<{ success: boolean; message: string; policy?: FrameworkEditorPolicyTemplate }> {
    ...
    if (!validationResult.success) {
        ...
        const formattedErrors = Object.entries(errorMessages)
            ...
            .join('; ');
        return { success: false, message: `Invalid data: ${formattedErrors || 'Please check your input.'}`, policy: undefined };
    }
    ...
    try {
        ...
        return { success: true, message: "Policy updated successfully.", policy: updatedPolicy };
    } catch (error) {
        console.error("Failed to update policy:", error);
        return { success: false, message: "Database error: Failed to update policy.", policy: undefined };
    }
}
...
export async function deletePolicyTemplate(
    ...
    try {
        ...
        return { success: false, message: "Database error: Failed to delete policy." };
    }
} 