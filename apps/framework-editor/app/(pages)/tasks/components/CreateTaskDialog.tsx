...
import { createTaskTemplateAction } from '../actions/create-task-template-action';
...
interface CreateTaskDialogProps {
  ...
  onTaskCreated: (task: FrameworkEditorTaskTemplate) => void;
}

export function CreateTaskDialog({ isOpen, onOpenChange, onTaskCreated }: CreateTaskDialogProps) {
  ...
  useEffect(() => {
    ...
    if (actionState.success && actionState.data) {
      toast.success(actionState.message || 'Task Template created successfully!');
      onOpenChange(false);
      form.reset();
      onTaskCreated(actionState.data);
  ...
  }, [actionState, onOpenChange, onTaskCreated, form]);
  ...
  async function onSubmit(values: TaskFormValues) {
    ...
    const result = await createTaskTemplateAction(undefined, formData);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the details below to create a new task template.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
