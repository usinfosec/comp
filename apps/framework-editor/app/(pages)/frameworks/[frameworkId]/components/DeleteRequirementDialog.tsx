...
import { AlertDialog, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from "@comp/ui/alert-dialog";
import { Button } from "@comp/ui/button";
import { useToast } from "@comp/ui/use-toast";
import { deleteRequirementAction, type DeleteRequirementActionState } from '../actions/delete-requirement-action';
  ...
  const handleSubmit = async () => {
    ...
    startTransition(async () => {
      ...
      if (result && !result.success) {
        const errorMessage = result.error || (result.issues ? result.issues.map(i => i.message).join(", ") : "Failed to delete requirement.");
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={handleDialogClose}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel disabled={isPending} onClick={() => handleDialogClose(false)}>Cancel</AlertDialogCancel>
