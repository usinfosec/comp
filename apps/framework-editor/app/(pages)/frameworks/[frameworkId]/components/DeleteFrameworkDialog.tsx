...
export function DeleteFrameworkDialog({
  ...
  const handleSubmit = async () => {
    ...
    startTransition(async () => {
      ...
      const result: DeleteFrameworkActionState = await deleteFrameworkAction(null, formData);

      if (result && !result.success) {
        setError(result.error || "Failed to delete framework.");
        toast({
          title: "Error",
          description: result.error || (result.issues ? result.issues.map(i => i.message).join(", ") : "An unexpected error occurred."),
          variant: "destructive",
        });
        ...
        toast({
          title: "Success",
          description: result.message || "Framework is being deleted. Redirecting...",
        });
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={(open) => {
      ...
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel disabled={isPending} onClick={() => onOpenChange(false)}>Cancel</AlertDialogCancel>
