	...
	for (const jsonFile of jsonFiles) {
		try {
			...
			if (subDirectory === "primitives") {
				...
				if (!zodSchema) {
					...
					for (const item of jsonData) {
						try {
							...
							throw new Error(
								`Data validation failed for ${jsonFile}.`,
							);
						}
					}
				...
				const modelNamesPart = jsonFile.substring(
					1,
					jsonFile.indexOf(".json"),
				);
				const [modelANamePascal, modelBNamePascal] =
					modelNamesPart.split("To");
				...
				const prismaModelAName =
					modelANamePascal.charAt(0).toLowerCase() +
				...
				let relationFieldNameOnModelA =
					modelBNamePascal.charAt(0).toLowerCase() +
...
async function main() {
	try {
		...
		console.log(
			"Seeding completed successfully for primitives and relations.",
		);
	} catch (error: unknown) {
