...
import type { Evidence, Frequency, Prisma } from "@prisma/client";
    ...
    try {
      ...
      await prisma.policy.upsert({
        ...
        update: {
          ...
          department: policyData.metadata?.department ?? Departments.none,
        },
        create: {
          id: policyData.metadata.id,
          slug: policyData.metadata.slug,
...
async function seedEvidenceRecords() {
  const evidenceDir = join(__dirname, "../../data/evidence");
  const evidenceFiles = readdirSync(evidenceDir).filter((file) =>
    file.endsWith(".json")
  );
  ...
  for (const file of evidenceFiles) {
    ...
    try {
      const fileContent = readFileSync(join(evidenceDir, file), "utf8");
      const evidenceData = JSON.parse(fileContent) as Evidence;
      ...
      await prisma.evidence.upsert({
        ...
        update: {
          ...
          department: evidenceData.department ?? Departments.none,
        },
        create: {
          id: evidenceData.id,
          name: evidenceData.name,
...
async function updateEvidenceLinks() {
  ...
  console.log(
    `🔄 Processing ${evidenceRequirements.length} evidence requirements`
  );
  ...
  for (const requirement of evidenceRequirements) {
    ...
    if (!control) {
      console.log(
        `  ⚠️ Control not found for requirement ${requirement.id}, skipping`
      );
      continue;
    }
    ...
    if (!control.frameworkCategory) {
      console.log(
        `  ⚠️ Framework category not found for control ${control.id}, skipping`
      );
      continue;
    }
    ...
    const controlsData = JSON.parse(
      fs.readFileSync(controlsFile, "utf8")
    ...
    if (!controlData) {
      console.log(
        `  ⚠️ Control data not found for ${control.code} in framework ${frameworkId}, skipping`
      );
      continue;
    }
    ...
    if (!reqData) {
      console.log(
        `  ⚠️ Requirement data not found for ${requirement.id} in control ${control.code}, skipping`
      );
      continue;
    }
    ...
    if (!evidenceId) {
      console.log(
        `  ⚠️ No evidenceId found for requirement ${requirement.id}, skipping`
      );
      continue;
    }
    ...
    if (!evidence) {
      console.log(
        `  ⚠️ Evidence ${evidenceId} not found for requirement ${requirement.id}, skipping`
      );
      continue;
    }

    console.log(
      `  ⏳ Linking requirement ${requirement.id} to evidence ${evidenceId}...`
    );
...
async function updatePolicyLinks() {
  ...
  for (const requirement of policyRequirements) {
    ...
    if (!policyId) {
      console.log(
        `  ⚠️ No policyId found for requirement ${requirement.id}, skipping`
      );
      continue;
    }
    ...
    if (!policy) {
      console.log(
        `  ⚠️ Policy ${policyId} not found for requirement ${requirement.id}, skipping`
      );
      continue;
    }

    console.log(
      `  ⏳ Linking requirement ${requirement.id} to policy ${policyId}...`
    );
