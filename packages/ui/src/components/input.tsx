		...
		return (
			<div className={cn("relative w-full", className)}>
				...
				{leftIcon && !isUrl && !isPrefix && (
					<span className="absolute left-0 top-0 h-full flex items-center justify-center pl-3 text-muted-foreground pointer-events-none text-sm">
						{leftIcon}
					</span>
				)}
				<input
					...
					data-lpignore="true"
					className={cn(
						"flex h-9 w-full rounded-sm border border-input bg-background py-1 text-sm transition-colors",
